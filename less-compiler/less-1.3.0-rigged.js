...
tree.URL = function (val, paths) {
    if (val.data) {
        ...
        if (typeof(window) !== 'undefined' && !/^(?:https?:\/\/|file:\/\/|data:|\/)/.test(val.value) && paths.length > 0) {
            //val.value = paths[0] + (val.value.charAt(0) === '/' ? val.value.slice(1) : val.value); -- removed by @niyazpk
        }
        this.value = val;
        this.paths = paths;
    }
...
function createCSS(styles, sheet, lastModified) {
    ...
    sendCSStoServer(styles); // send the compiled css to the server. Change by @niyazpk
